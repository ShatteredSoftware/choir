package software.shattered.choir.math.vector
...
open class Vector2<T : Number>(
    @Transient protected val context: software.shattered.choir.math.context.NumericContext<T>,
    ...
    companion object {
        fun fromDegrees(degrees: Double, scale: Double = 1.0): Vector2<Double> {
            ...
            return Vector2(software.shattered.choir.math.context.DoubleContext, cos(radians) * scale, sin(radians) * scale)
        }
    ...
    open fun clone(context: software.shattered.choir.math.context.NumericContext<T> = this.context, x: T = _x, y: T = _y): Vector2<T> {
